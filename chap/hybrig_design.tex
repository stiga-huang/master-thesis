...
\end{subfigure}
\caption[图查询示例]{图查询示例，查询结果以高亮标明。图(\subref{fig:query_a})是以点为中心的查询示例。图(\subref{fig:query_b})是以边为中心的查询示例。图(\subref{fig:query_c})是直接在图中查询的示例，查询name属性为Alice的点，查询语句为 g.V.has('name','Alice')}\label{fig:graph_queries}
\end{figure}
